...
function LocalEntities() {
    ...
    ////////////////////////// Inicialização
        ...
        function activate() {
            ...
            ledb_lastIds.iterate(function (value, key) {
                ...
                isReady = true;
                while (regReqs.length > 0) {
                    var reg = regReqs.shift();
                    _register(reg.constr, reg.mapping);
                }
                while (getAllReqs.length > 0) {
                    var get = getAllReqs.shift();
                    resolveGetAll(get.type, get.include, get.resolve, get.reject);
                }
            });
        }
    ...
    ////////////////////////// Funções públicas
        ...
        function register(constr, mapping) {
            if (!isReady)
                regReqs.push({ constr: constr, mapping: mapping });
            else
